 ...
 */
export const purgeOffsets = functions.https.onCall(
  async (data: PurgeOffsetsRequest, context) => {
    // Restrict to authenticated admin users only
    const userRole = (context.auth?.token as any)?.role;
    ...
    try {
      ...
      if (
        !globalState ||
        !globalState.team ||
        !Array.isArray(globalState.team)
      ) {
      ...
      await globalStateRef.update({
        team: cleanedTeam,
        lastOffsetPurge: admin.firestore.Timestamp.now(),
      });
